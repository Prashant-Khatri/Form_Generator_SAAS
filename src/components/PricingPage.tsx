...
import { PricingPlan, pricingPlan } from "@/lib/pricing-plan";
            ...
            <div className="grid grid-cols-3 gap-6">
                {
                    pricingPlan.map((plan: PricingPlan, index: number) => (
                        <Card className={`${plan.level === "Enterprise" && "bg-[#1c1c1c] text-white"} flex flex-col justify-between w-87.5`} key={index}>
                            ...
                            <CardFooter>
                                <Button variant={"outline"} className={`${plan.level === "Enterprise" && "text-black"} w-full`}>Get started with {plan.level}</Button>
                            </CardFooter>
                        </Card>
                    ))
                }
